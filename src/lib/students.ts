 ...
 * is a Clock In with no matching Clock Out yet, along with their supervisor's
 * contact info. Used by the 10pm cron check - callers should flag each
 * returned record's attendance_log row after emailing so a retry doesn't
 * send a duplicate.
 */
export async function getStudentsMissingClockOutToday(): Promise<MissedClockOutRow[]> {
  const rows = await sql`
    SELECT s.id AS student_id, s.name AS student_name, last_event.id AS attendance_log_id,
           u.id AS owner_id, COALESCE(u.name, u.email) AS owner_name, u.email AS owner_email
    FROM students s
    JOIN users u ON u.id = s.owner_id
    JOIN LATERAL (
